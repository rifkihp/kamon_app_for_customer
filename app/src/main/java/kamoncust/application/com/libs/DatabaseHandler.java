...
public class DatabaseHandler extends SQLiteOpenHelper {
	...
	private static final String DATABASE_NAMA = "kamoncust";
	public static final String TABLE_CART = "cart";
	public static final String TABLE_WISHLIST = "wishlist";

	public static final String TABLE_ALAMAT = "alamat";
	public static final String TABLE_JADWAL_TERAPI = "jadwalterapi";
	...
	public void createTable() {
		...
		//db.execSQL("DROP TABLE IF EXISTS " + TABLE_JADWAL_TERAPI);
		db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_JADWAL_TERAPI+ "(" +
	...
	public alamat getAlamat() {
		...
		alamat select_alamat = new alamat(0, "", "",0, 0, 0, "", 0, "", 0, "", "", "", false, false, "", "", "");
		try {
			String sql = "SELECT nama, alamat, latitude, longitude, province_id, province, city_id, city_name, subdistrict_id, subdistrict_name, kode_pos, no_hp, is_dropship, dropship_name, dropship_phone, email_notifikasi, kurir_id, kurir, layanan, etd, tarif, nominal FROM " + TABLE_ALAMAT;
			SQLiteDatabase db = this.getReadableDatabase();
			Cursor cursor = db.rawQuery(sql, null);
			if(cursor.getCount() > 0) {
				cursor.moveToFirst();
				for(int i=0; i<cursor.getCount(); i++) {
					select_alamat = new alamat(0, cursor.getString(0), cursor.getString(1), cursor.getDouble(2), cursor.getDouble(3), cursor.getInt(4), cursor.getString(5), cursor.getInt(6), cursor.getString(7), cursor.getInt(8), cursor.getString(9), cursor.getString(10), cursor.getString(11), false, cursor.getInt(12)==1, cursor.getString(13), cursor.getString(14), cursor.getString(15));
					cursor.moveToNext();
				}
			}
			cursor.close();
	...
	public jadwal getJadwalTerapi() {

		jadwal select_jadwal = new jadwal(0, "", "","");
		try {
			String sql = "SELECT tanggal_terapi, jam_terapi, riwayat_kesehatan FROM " + TABLE_JADWAL_TERAPI;
			SQLiteDatabase db = this.getReadableDatabase();
			Cursor cursor = db.rawQuery(sql, null);
			if(cursor.getCount() > 0) {
				cursor.moveToFirst();
				for(int i=0; i<cursor.getCount(); i++) {
					select_jadwal = new jadwal(0, cursor.getString(0), cursor.getString(1), cursor.getString(2));
					cursor.moveToNext();
				}
			}
			cursor.close();
		...
		return select_jadwal;
	}

	public void insertJadwalTerapi(jadwal data) {
		deleteJadwalTerapi();

		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put("tanggal_terapi", data.getTanggal_terapi());
		values.put("jam_terapi", data.getJam_terapi());
		values.put("riwayat_kesehatan", data.getRiwayat_kesehatan());

		db.insert(TABLE_JADWAL_TERAPI, null, values);
		db.close();
	}

	public void deleteJadwalTerapi() {
		SQLiteDatabase db = this.getWritableDatabase();
		db.delete(TABLE_JADWAL_TERAPI, null, null);
		db.close();
	}
	...
	public grandtotal getGrandtotal() {
		...
		try {
			String sql = "SELECT total, diskon, subtotal, voucher, ongkir, grandtotal FROM " + TABLE_GRANDTOTAL;
			SQLiteDatabase db = this.getReadableDatabase();
			Cursor cursor = db.rawQuery(sql, null);
			if(cursor.getCount() > 0) {
				cursor.moveToFirst();
	...
	public void insertGrandtotal(grandtotal data) {
		...
		values.put("subtotal", data.getSubtotal());
		values.put("voucher", data.getVoucher());
		values.put("ongkir", data.getOngkir());
		values.put("grandtotal", data.getGrandtotal());
	...
	public int getIdWishlist(int id) {
		...
		try {
			String sql = "SELECT id FROM " + TABLE_WISHLIST + " WHERE id="+id;
			SQLiteDatabase db = this.getReadableDatabase();
			Cursor cursor = db.rawQuery(sql, null);
			if(cursor.getCount() > 0) {
				cursor.moveToFirst();
	...
	public int getIdWishlist(produk item) {
		int result = 0; //jika return 0 maka data bukan wishlist;
		try {
			String sql = "SELECT id FROM " + TABLE_WISHLIST + " WHERE id="+item.getId();
			SQLiteDatabase db = this.getReadableDatabase();
			Cursor cursor = db.rawQuery(sql, null);
			if(cursor.getCount() > 0) {
				cursor.moveToFirst();
	...
	public void insertWishlists(ArrayList<produk> wishlist_items) {
		...
		for(produk wishlist_item: wishlist_items) {
			...
			values.put("responden", wishlist_item.getResponden());
			values.put("review", wishlist_item.getReview());
